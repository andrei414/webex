...
node {
    withEnv(["HOME=${env.WORKSPACE}"]) {
        withEnv(["PATH+sfx=${WORKSPACE}/sfdx/bin"]) {
			withEnv(["HUB_ORG_DH=conttest414-aad2@force.com"]){
				withEnv(["SFDC_HOST_DH=https://login.salesforce.com"]){
					withEnv(["CONNECTED_APP_CONSUMER_KEY_DH=3MVG9SOw8KERNN0.kF.gZhK.3VoVL65c2VncoLTiigo1vv50w4m8GQiUw5VoIURBHzBvmt9W_u_NyvEM7ES78"]){
						withEnv(["JWT_CRED_ID_DH=3e95ac4d-61d5-4b8c-937c-8050ccd12b89"]){
							stage('Install Sfdx') {
								sh '''
